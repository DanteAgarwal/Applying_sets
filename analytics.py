...
#     fig = px.line(applications_over_time, x='date_applied', y='Count', title='Applications Over Time', labels={'Count': 'Number of Applications', 'date_applied': 'Month'})
#     st.plotly_chart(fig)
...
#     fig = px.bar(avg_time_to_follow_up, x='status', y='time_to_follow_up', title='Average Time to Follow-up by Status', labels={'time_to_follow_up': 'Average Days to Follow-up', 'status': 'Status'}, color='time_to_follow_up', color_continuous_scale='Viridis')
#     st.plotly_chart(fig)
...
#     with col1:
#         fig = px.bar(company_counts, x='Count', y='Company', orientation='h', title='Top 5 Companies Applied To', color='Count', color_continuous_scale='Blues')
#         st.plotly_chart(fig)

#     with col2:
#         fig = px.bar(job_title_counts, x='Count', y='Job Title', orientation='h', title='Top 5 Job Titles Applied To', color='Count', color_continuous_scale='Greens')
#         st.plotly_chart(fig)
...
#     fig = px.bar(conversion_rates, x='status', y='Conversion Rate (%)', title='Conversion Rates by Status', labels={'Conversion Rate (%)': 'Conversion Rate (%)', 'status': 'Status'}, color='Conversion Rate (%)', color_continuous_scale='Viridis')
#     st.plotly_chart(fig)
...
def get_colorscale(name):
    custom_scales = {
        'Salmon': [[0.0, 'rgb(255, 229, 229)'], [0.5, 'rgb(255, 160, 122)'], [1.0, 'rgb(233, 87, 63)']],
        'Cool': [[0.0, 'rgb(0, 255, 255)'], [0.5, 'rgb(127, 127, 255)'], [1.0, 'rgb(255, 0, 255)']],
        'Plasma': 'plasma',
        'Sunset': 'sunset',
        'Viridis': 'viridis',
        'Inferno': 'inferno',
...
def plot_bar(df, x, y, title, color_col=None, orientation='v', color_map='Viridis'):
    return px.bar(df, x=x, y=y, title=title, color=color_col or y,
                  orientation=orientation, color_continuous_scale=color_map)
...
class JobAnalyticsEngine:
        ...
        with st.expander("💡 Personalized Insights", expanded=True):
            for label, emoji in {
                ...
                if label in self.status_counts['Status'].values:
                    ...
                    if count:
                        func = st.success if label == 'Offer Received' else \
                               st.info if label == 'Interview Scheduled' else \
                               st.warning if label == 'Ghosted' else st.error
    ...
    def show_status_priority(self):
        with st.expander("📌 Application Status & Priorities"):
            ...
            col1, col2 = st.columns(2)
            col1.plotly_chart(plot_bar(status_df, 'Status', 'Count', 'Status Distribution', color_map=get_colorscale('Turbo')), use_container_width=True)
            col2.plotly_chart(plot_bar(priority_df, 'Priority', 'Count', 'Priority Distribution', color_map=get_colorscale('Magma')), use_container_width=True)

    def show_timeline(self):
        with st.expander("📅 Timeline Analysis"):
            trend = self.filtered_df.groupby(self.filtered_df['date_applied'].dt.to_period('M')).size().reset_index(name='Applications')
            trend['date_applied'] = trend['date_applied'].astype(str)
            st.plotly_chart(px.line(trend, x='date_applied', y='Applications', title='Applications Over Time', markers=True), use_container_width=True)
    ...
    def show_followups(self):
        with st.expander("⏱️ Follow-up Metrics"):
            ...
            self.filtered_df['time_to_follow_up'] = (self.filtered_df['follow_up_date'] - self.filtered_df['date_applied']).dt.days
            followup = self.filtered_df.dropna(subset=['time_to_follow_up']).groupby('status')['time_to_follow_up'].mean().reset_index()
            st.plotly_chart(plot_bar(followup, 'status', 'time_to_follow_up', 'Avg Days to Follow-up by Status', color_map=get_colorscale('Salmon')), use_container_width=True)
    ...
    def show_top_targets(self):
        with st.expander("🏢 Top Applications Targets"):
            ...
            col1.plotly_chart(plot_bar(company_df, 'Count', 'Company', 'Top 10 Companies', orientation='h', color_map=get_colorscale('Cool')), use_container_width=True)
            col2.plotly_chart(plot_bar(job_df, 'Count', 'Job Title', 'Top 10 Job Titles', orientation='h', color_map=get_colorscale('Plasma')), use_container_width=True)
    ...
    def show_conversion(self):
        with st.expander("🌡️ Conversion & Ghosting Rate"):
            ...
            st.plotly_chart(plot_bar(conversion_df, 'Status', 'Conversion Rate (%)', 'Conversion Rate by Status', color_map=get_colorscale('Cyan')), use_container_width=True)
    ...
    def show_heatmap(self):
        with st.expander("📅 Heatmap of Applications"):
            ...
            pivot = heatmap.pivot_table(index='month_applied', columns='status', aggfunc='size', fill_value=0).T
            st.plotly_chart(px.imshow(pivot, aspect="auto", title='Monthly Status Heatmap', color_continuous_scale='YlGnBu'), use_container_width=True)
    ...
    def show_reminders(self):
        with st.expander("🔔 Follow-up Reminders & Recents"):
            ...
            col1.dataframe(upcoming[['company_name', 'job_title', 'follow_up_date', 'notes']]) if not upcoming.empty else col1.write("No upcoming follow-ups.")
...
def analytics_ui(conn):
    ...
    st.sidebar.header("🧮 Filter Options")
    status_filter = st.sidebar.multiselect("Select Status", engine.df['status'].unique(), default=list(engine.df['status'].unique()))
    priority_filter = st.sidebar.multiselect("Select Priority", engine.df['priority'].unique(), default=list(engine.df['priority'].unique()))
    date_range = st.sidebar.date_input("Select Date Range", [engine.df['date_applied'].min(), engine.df['date_applied'].max()])

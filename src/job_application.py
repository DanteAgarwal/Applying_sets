        ...
        with st.form("job_form"):
            ...
            if submit_button:
                if not company_name or not job_title:
                    ...
                    st.success(
                        f"✅ Application for *{job_title}* at *{company_name}* saved!"
                    )
                    st.balloons()
...
class JobManager:
    ...
    def view_update_ui(self):
        ...
        filtered_jobs = jobs[
            (
                jobs["company_name"].str.contains(search_text, case=False)
                | jobs["job_title"].str.contains(search_text, case=False)
            )
        ...
        with st.expander("💡 Jobs Cards", expanded=True):
            ...
            for _, job in current_jobs.iterrows():
                ...
                job_card.render()
            st.caption(
                f"Page {page} of {total_pages} | Showing {start_idx + 1}-{min(end_idx, total_jobs)} of {total_jobs} jobs."
            )
        ...
        application_id = st.number_input(
            "Enter Application ID to Update/Delete", min_value=1
        )
                ...
                with st.form("update_form"):
                    new_status = st.selectbox(
                        ...
                        index=[
                            ...
                            "Rejected",
                            "Ghosted",
                        ].index(application["status"].values[0]),
                    )
                    new_follow_up_date = st.date_input(
                        "Update Follow-up Date",
                        pd.to_datetime(application["follow_up_date"].values[0]),
                    )
                    ...
                    new_interview_date = st.date_input(
                        ...
                        (
                            pd.to_datetime(interview_date_val)
                            if pd.notna(interview_date_val)
                            else datetime.today()
                        ),
                    )
                    new_notes = st.text_area(
                        "Update Notes", application["notes"].values[0]
                    )
